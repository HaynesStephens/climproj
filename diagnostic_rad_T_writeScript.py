...
def createRun(input_ppm):
    irradiance = 1036
    insol = 320
    job_name = 'diagnostic_rad_T_input{0}'.format(input_ppm)

    template_path = '/home/haynes13/code/python/climproj/climt_scripts/diagnostic/rad/diagnostic_rad_T_template.py'
    job_path = '/home/haynes13/code/python/climproj/climt_scripts/diagnostic/rad/T/{0}.py'.format(job_name)
    print(job_path)
    ...
    test_dir = 'diagnostic/rad/T/' # Needs to end in an '/'
    job_dir, sbatch_filename = newSbatch.newSbatch(base_dir, test_dir, job_name)
    return job_dir, sbatch_filename

os.system('mkdir -p /home/haynes13/code/python/climproj/climt_scripts/diagnostic/rad/T/')
