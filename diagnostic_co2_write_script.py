...
def createRun(input_ppm):
    irradiance = 1036
    insol = 320
    job_name = 'diagnostic_co2_input{0}'.format(input_ppm)

    template_path = '/home/haynes13/code/python/climproj/climt_scripts/diagnostic/diagnostic_co2_template.py'
    job_path = '/home/haynes13/code/python/climproj/climt_scripts/diagnostic/co2/{0}.py'.format(job_name)
    print(job_path)
    ...
    test_dir = 'diagnostic/co2/' # Needs to end in an '/'
    job_dir, sbatch_filename = newSbatch.newSbatch(base_dir, test_dir, job_name)
    return job_dir, sbatch_filename

os.system('mkdir -p /home/haynes13/code/python/climproj/climt_scripts/diagnostic/co2/')
...
for input_ppm in input_ppm_list:
    ...
    os.chdir(job_dir)
    os.system('sbatch {0}'.format(sbatch_filename)) love
